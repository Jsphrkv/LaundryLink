...
import { Button, StatusBadge, Card, EmptyState, PageSpinner, StarRating, Modal } from '../../components/ui';
...
export function OrdersPage() {
  ...
  useEffect(() => {
    if (!user) return;
    orderService.getCustomerOrders(user.id).then(setOrders).finally(() => setLoading(false));
  }, [user]);
  ...
  if (loading) return <DashboardLayout title="My Orders"><PageSpinner /></DashboardLayout>;
    ...
    <DashboardLayout title="My Orders">
      ...
      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl mb-6 w-full max-w-sm">
        {(['active', 'history'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={clsx('flex-1 py-2 rounded-lg text-sm font-bold transition-all capitalize',
      ...
      {shown.length === 0 ? (
        <EmptyState
          ...
          subtitle={tab === 'active' ? 'Book a laundry pickup to get started!' : 'Your completed orders will appear here.'}
...
export function OrderDetailPage() {
  ...
  const handleCancel = async () => {
    ...
    try {
      await orderService.updateOrderStatus(order.id, 'cancelled', user.id, 'Cancelled by customer');
  ...
  const handleRate = async () => {
    ...
    setRating(true);
    try {
      await orderService.submitRating(order.id, user.id, shopStar, riderStar || null, '', '');
      setShowRate(false);
  ...
  if (!order)  return <DashboardLayout><EmptyState emoji="❓" title="Order not found" /></DashboardLayout>;
    ...
    <DashboardLayout title={`Order ${order.order_number}`}>
      ...
      <Card className="p-4 mb-4">
        <div className="flex items-start justify-between">
          <div>
            <p className="font-extrabold text-lg text-gray-900">{order.order_number}</p>
            <p className="text-sm text-gray-400 mt-0.5">{(order as any).shop?.shop_name}</p>
          </div>
              ...
              return (
                <div key={s.status} className="flex items-start gap-3">
                  <div className="flex flex-col items-center">
                    <div className={clsx('w-8 h-8 rounded-full flex items-center justify-center text-sm border-2 transition-all',
                      done ? (current ? 'bg-primary border-primary text-white' : 'bg-green-100 border-green-400 text-green-600')
                           : 'bg-gray-50 border-gray-200 text-gray-300')}>
                      {done ? s.icon : '○'}
                    </div>
                    {i < STATUS_TIMELINE.length - 1 && (
                      <div className={clsx('w-0.5 h-6', done && !current ? 'bg-green-300' : 'bg-gray-200')} />
                    )}
                  </div>
        ...
        <Card className="p-4 mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-bold text-gray-900">{(order as any).rider?.user?.full_name ?? 'Rider'}</p>
              <p className="text-xs text-gray-400">
                {(order as any).rider?.vehicle_type} · {(order as any).rider?.vehicle_plate}
              </p>
            </div>
            <a href={`tel:${(order as any).rider?.user?.phone}`}
      ...
      <Card className="divide-y divide-gray-50 mb-4">
        ...
        {[
          ...
          ['Est. Weight', `~${order.estimated_weight_kg} kg${order.actual_weight_kg ? ` → ${order.actual_weight_kg}kg actual` : ''}`],
      ...
      <Card className="divide-y divide-gray-50 mb-4">
        ...
        <div className="flex justify-between px-4 py-3">
          ...
          <span className="text-sm font-semibold">{APP_CONFIG.currency}{order.subtotal.toFixed(2)}</span>
        </div>
        <div className="flex justify-between px-4 py-3">
          <span className="text-sm text-gray-500">Delivery Fee</span>
          <span className="text-sm font-semibold">{APP_CONFIG.currency}{order.delivery_fee.toFixed(2)}</span>
        </div>
        <div className="flex justify-between px-4 py-3">
          <span className="font-bold text-gray-900">Total</span>
          <span className="text-xl font-extrabold text-primary">{APP_CONFIG.currency}{order.total_amount.toFixed(2)}</span>
        </div>
        <div className="flex justify-between px-4 py-3">
        ...
        <div className="flex justify-between px-4 py-3">
          ...
          <span className={clsx('text-sm font-bold', order.payment_status === 'paid' ? 'text-green-600' : 'text-amber-600')}>
      ...
      {/* Rating Modal */}
      <Modal open={showRate} onClose={() => setShowRate(false)} title="Rate Your Experience">
        <div className="space-y-4">
          <StarRating label="Laundry Shop" value={shopStar} onChange={setShopStar} />
          {order.rider_id && (
            <StarRating label="Rider" value={riderStar} onChange={setRiderStar} />
          )}
          <div className="flex gap-3 pt-2">
            <Button variant="ghost" onClick={() => setShowRate(false)} fullWidth>Skip</Button>
            <Button onClick={handleRate} loading={rating} fullWidth>Submit</Button>
          </div>
        </div>
      </Modal>
    </DashboardLayout>
