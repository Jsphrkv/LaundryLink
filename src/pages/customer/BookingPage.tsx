...
import { MapPin, Calendar, Scale, Shirt, Store, CreditCard, CheckCircle, ArrowLeft, Loader2 } from 'lucide-react';
...
import { SERVICE_META, SERVICE_LABELS, TIME_SLOTS, APP_CONFIG } from '../../constants';
            ...
            return (
              <React.Fragment key={i}>
                <div className={clsx('flex flex-col items-center gap-1 shrink-0',
                  current ? 'opacity-100' : done ? 'opacity-100' : 'opacity-40')}>
                  <div className={clsx('w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all',
                    done ? 'bg-success text-white' : current ? 'bg-primary text-white' : 'bg-gray-100 text-gray-400')}>
                    {done ? '✓' : i + 1}
                  </div>
                  <span className={clsx('text-[10px] font-semibold whitespace-nowrap',
                    current ? 'text-primary' : done ? 'text-success' : 'text-gray-400')}>
                    {s.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div className={clsx('h-0.5 flex-1 mx-1 mt-[-12px] rounded transition-all',
...
function StepAddress() {
  ...
  const [suggestions, setSuggestions] = useState<{ place_name: string; lat: number; lng: number }[]>([]);
  const [adding,    setAdding]    = useState(false);

  useEffect(() => {
    if (!user) return;
    addressService.getUserAddresses(user.id).then(a => {
      setAddresses(a);
  ...
  const handleSave = async () => {
    if (!fullAddr || !user) { toast.error('Enter a full address'); return; }
    setAdding(true);
    try {
      ...
      const addr = await addressService.addAddress(user.id, {
        ...
        barangay: '', city: APP_CONFIG.defaultCity, province: 'Metro Manila',
        ...coords,
        is_default: addresses.length === 0,
      });
      setAddresses(p => [...p, addr]);
      booking.setPickupAddress(addr);
      setFullAddr(''); setLabel(''); setSuggestions([]);
  ...
  return (
    <StepCard title="📍 Pickup Address" desc="Where should we pick up your laundry?">
      {loading ? (
        ...
        <>
          {addresses.map(addr => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-400 truncate">{addr.full_address}</p>
          ...
          <div className="mt-4 p-4 border-2 border-dashed border-gray-200 rounded-xl">
                ...
                <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-xl shadow-elevated -mt-3 mb-2 overflow-hidden">
                  {suggestions.map((s, i) => (
                    <button
                      key={i}
                      type="button"
                      onClick={() => { setFullAddr(s.place_name); setSuggestions([]); }}
            ...
            <Button variant="outline" onClick={handleSave} loading={adding} fullWidth>
              Save Address
            </Button>
          </div>
        </>
      ...
      <StepNav
        onNext={() => {
          if (!booking.pickup_address) { toast.error('Please select a pickup address'); return; }
          booking.nextStep();
        }}
        hideBack
      />
...
function StepSchedule() {
  ...
  return (
    <StepCard title="📅 Schedule Pickup" desc="When should we pick up your laundry?">
      <div className="mb-5">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-thin">
          {dates.map(d => {
            ...
            return (
              <button key={str} onClick={() => setSelDate(str)}
                className={clsx('flex flex-col items-center px-4 py-3 rounded-xl border-2 transition-all shrink-0 min-w-[64px]',
                  active ? 'border-primary bg-primary text-white' : 'border-gray-200 hover:border-primary-300')}>
                <span className={clsx('text-xs font-semibold', active ? 'text-white/80' : 'text-gray-400')}>{format(d, 'EEE')}</span>
                <span className={clsx('text-xl font-extrabold', active ? 'text-white' : 'text-gray-900')}>{format(d, 'd')}</span>
                <span className={clsx('text-xs', active ? 'text-white/80' : 'text-gray-400')}>{format(d, 'MMM')}</span>
              </button>
            );
          })}
        </div>
        ...
        <div className="space-y-2">
          {TIME_SLOTS.map(slot => (
            <button key={slot} onClick={() => setSelTime(slot)}
              className={clsx('w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 text-sm font-semibold transition-all',
                selTime === slot
      ...
      <StepNav onNext={() => {
        if (!selDate || !selTime) { toast.error('Please select a date and time'); return; }
...
function StepLaundry() {
  ...
  const counter = (val: number, set: (v: number) => void, step = 1, min = 1, max = 20) => (
    <div className="flex items-center gap-3">
      <button onClick={() => set(Math.max(min, val - step))}
        className="w-9 h-9 rounded-full bg-primary-50 text-primary font-bold text-lg flex items-center justify-center hover:bg-primary-100 transition">−</button>
      <span className="text-xl font-extrabold text-gray-900 min-w-[60px] text-center">{val}{step < 1 ? ' kg' : ''}</span>
  ...
  return (
    <StepCard title="⚖️ Laundry Details" desc="Tell us about your laundry load">
      ...
      <Textarea label="Special Instructions (Optional)" value={notes}
        onChange={e => setNotes(e.target.value)}
        placeholder="e.g. Separate whites from colors, handle with care..." rows={3} />
  ...
  return (
    <StepCard title="👕 Service Type" desc="What type of service do you need?">
      <div className="space-y-3">
        {(Object.entries(SERVICE_META) as [ServiceType, typeof SERVICE_META[keyof typeof SERVICE_META]][]).map(([type, meta]) => (
          <div
            key={type}
            onClick={() => setSel(type)}
            className={clsx('flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all',
              sel === type ? 'border-primary bg-primary-50' : `border-gray-100 ${meta.color} hover:border-primary-200`)}
          >
            <span className="text-3xl">{meta.icon}</span>
            <div className="flex-1">
              <p className={clsx('font-bold', sel === type ? 'text-primary' : 'text-gray-900')}>
                {SERVICE_LABELS[type]}
              </p>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-xs text-gray-500 bg-white/70 px-2 py-0.5 rounded-full">⏱ {meta.eta}</span>
                <span className="text-xs text-gray-500 bg-white/70 px-2 py-0.5 rounded-full">{meta.hint}</span>
              </div>
            </div>
            {sel === type && <CheckCircle className="text-primary shrink-0" size={20} />}
          </div>
        ))}
      </div>

      <StepNav onNext={() => {
        if (!sel) { toast.error('Please select a service type'); return; }
  ...
  const load = async (f: ShopFilter) => {
    ...
    try {
      const result = await shopService.getNearbyShops(
        APP_CONFIG.defaultLat, APP_CONFIG.defaultLng, f, booking.service_type
      );
      setShops(result);
    } catch { setShops([]); }
    finally { setLoading(false); }
  ...
  return (
    <StepCard title="🏪 Choose a Shop" desc="Select a nearby laundry shop">
      <div className="flex gap-2 mb-4">
        {FILTERS.map(f => (
          <button key={f.key} onClick={() => { setFilter(f.key); booking.setShopFilter(f.key); }}
            className={clsx('flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-sm font-bold border-2 transition-all',
              filter === f.key ? 'border-primary bg-primary text-white' : 'border-gray-200 text-gray-600 hover:border-primary-300')}>
            {f.emoji} {f.label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState emoji="🏪" title="No shops available" subtitle="Try changing the filter or check back later" />
      ) : (
        <div className="space-y-3">
          {shops.map(shop => (
      ...
      <StepNav onNext={() => {
        if (!booking.selected_shop) { toast.error('Please select a shop'); return; }
        booking.nextStep();
      }} onBack={booking.prevStep} />
    </StepCard>
  );
...
function StepPayment() {
  const booking = useBookingStore();
  const [method, setMethod] = useState<'cash_on_delivery' | 'gcash' | 'paymaya'>(
    (booking.payment_method as any) ?? 'cash_on_delivery'
  );

  const METHODS = [
    { key: 'cash_on_delivery', icon: '💵', label: 'Cash on Delivery', desc: 'Pay when laundry arrives', disabled: false },
    { key: 'gcash',   icon: '📱', label: 'GCash',   desc: 'Coming soon', disabled: true },
    { key: 'paymaya', icon: '💳', label: 'PayMaya', desc: 'Coming soon', disabled: true },
  ];
  ...
  return (
    <StepCard title="💳 Payment Method" desc="How would you like to pay?">
        ...
        {METHODS.map(m => (
          <div key={m.key}
            ...
            className={clsx(
              ...
              method === m.key ? 'border-primary bg-primary-50' : 'border-gray-100 hover:border-gray-200'
            )}
          >
            <span className="text-3xl">{m.icon}</span>
            <div className="flex-1">
              <p className={clsx('font-bold', method === m.key ? 'text-primary' : 'text-gray-900')}>{m.label}</p>
              <p className="text-xs text-gray-400 mt-0.5">{m.desc}</p>
            </div>
            {m.disabled
              ? <span className="text-xs bg-gray-100 text-gray-500 font-bold px-2 py-1 rounded-full">Soon</span>
              : method === m.key
      ...
      <StepNav onNext={() => { booking.setPaymentMethod(method); booking.nextStep(); }} onBack={booking.prevStep} />
    </StepCard>
  );
}
...
function StepConfirm() {
  ...
  const service   = shop?.services?.find(s => s.service_type === booking.service_type);
  const subtotal  = (service?.price_per_kg ?? 0) * booking.estimated_weight_kg;
  const delFee    = APP_CONFIG.deliveryFeeBase + (shop?.distance_km ?? 3) * APP_CONFIG.deliveryFeePerKm;
  ...
  const rows: [string, string][] = [
    ...
    ['Payment',      booking.payment_method === 'cash_on_delivery' ? '💵 Cash on Delivery' : '📱 GCash'],
  ];

  return (
    <StepCard title="✅ Confirm Booking" desc="Review your booking details before placing">
      <Card className="divide-y divide-gray-50 mb-4">
        ...
        {booking.special_instructions && (
          <div className="flex items-start justify-between px-4 py-3 gap-4">
            ...
            <span className="text-sm font-semibold text-gray-900 text-right">{booking.special_instructions}</span>
          </div>
        )}
        <div className="flex items-center justify-between px-4 py-3">
          <span className="text-sm text-gray-500">Subtotal</span>
          <span className="text-sm font-semibold">{APP_CONFIG.currency}{subtotal.toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between px-4 py-3">
          <span className="text-sm text-gray-500">Delivery Fee</span>
          <span className="text-sm font-semibold">{APP_CONFIG.currency}{delFee.toFixed(2)}</span>
        </div>
        <div className="flex items-center justify-between px-4 py-3">
          <span className="text-base font-bold text-gray-900">Total</span>
          <span className="text-xl font-extrabold text-primary">{APP_CONFIG.currency}{total.toFixed(2)}</span>
        </div>
      </Card>

      <div className="flex gap-3">
        <Button variant="outline" onClick={booking.prevStep} leftIcon={<ArrowLeft size={16} />}>
          Back
        </Button>
        <Button fullWidth loading={loading} onClick={handleConfirm} leftIcon={<CheckCircle size={16} />}>
          Place Order
        </Button>
      </div>
    </StepCard>
...
function StepNav({ onNext, onBack, hideBack }: { onNext: () => void; onBack?: () => void; hideBack?: boolean }) {
  return (
