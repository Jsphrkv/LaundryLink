    ...
    >
      ...
      <div className="relative my-6">
        ...
        <div className="relative flex justify-center">
          <span className="bg-white px-3 text-xs text-gray-400">New to LaundryLink?</span>
        </div>
      </div>
...
// ─── Register Page ────────────────────────────────────────────────────────────
const ROLES = [
  { value: 'customer',   label: 'Customer',   icon: '👤', desc: 'Book laundry pickups' },
  { value: 'rider',      label: 'Rider',       icon: '🛵', desc: 'Deliver laundry orders' },
  { value: 'shop_owner', label: 'Shop Owner',  icon: '🏪', desc: 'List your laundry shop' },
];
...
export function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!fullName || !email || !phone || !password) { toast.error('Please fill in all fields'); return; }
    if (password !== confirm) { toast.error('Passwords do not match'); return; }
    if (password.length < 8)  { toast.error('Password must be at least 8 characters'); return; }
    try {
      await signUp(email.trim().toLowerCase(), password, fullName.trim(), phone.trim(), role);
  ...
  return (
    <AuthLayout title="Create account" subtitle="Join LaundryLink today">
      {/* Role selector */}
      <div className="mb-5">
        <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">I want to join as</p>
        <div className="grid grid-cols-3 gap-2">
          {ROLES.map(r => (
            <button
              key={r.value}
            ...
            >
              <span className="text-xl">{r.icon}</span>
              <span className={`text-xs font-bold ${role === r.value ? 'text-primary' : 'text-gray-600'}`}>
                {r.label}
              </span>
      ...
      <form onSubmit={handleSubmit} className="space-y-1">
        <Input label="Full Name" value={fullName} onChange={e => setFullName(e.target.value)}
          placeholder="Juan dela Cruz" leftIcon={<User size={16} />} />
        <Input label="Email Address" type="email" value={email} onChange={e => setEmail(e.target.value)}
          placeholder="you@example.com" leftIcon={<Mail size={16} />} />
        <Input label="Phone Number" type="tel" value={phone} onChange={e => setPhone(e.target.value)}
          placeholder="+63 912 345 6789" leftIcon={<Phone size={16} />} />
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary-700 via-primary to-secondary p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/70 text-sm mt-1">Fresh clothes, delivered to you</p>
        </div>
